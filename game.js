...
const BLOCK_COLORS = {
  cobble: 0x888888, planks: 0xC4A35A, dirt: 0x8B6914,
  sand: 0xF4D03F, stone: 0x777777, wood: 0x5C3317,
  leaf: 0x228B22, lava: 0xFF4500, glass: 0x88ccff,
};
...
function makeVillageWall() {
  ...
  for (let i = -R; i <= R; i++) {
    ...
    for (let h = 0; h < 3; h++) {
      placeBlock(i, h+0.5, -R, 'cobble'); placeBlock(i, h+0.5, R, 'cobble');
      placeBlock(-R, h+0.5, i, 'cobble'); placeBlock(R, h+0.5, i, 'cobble');
    }
  }
}
...
function generateWorld() {
  ...
  makeFountain(0, 0);
  makeVillageWall();
  [[-12,-10],[12,-10],[-12,10],[12,10],[-12,0],[12,0],[0,-14],[0,14]].forEach(([hx,hz]) => makeHouse(hx,hz));
}
...
renderer.domElement.addEventListener('click', () => {
  ...
  if (hit) {
    ...
    if (pos.distanceTo(camera.position) > 1.2) placeBlock(pos.x, pos.y, pos.z, HOTBAR[selectedSlot]);
  }
});
...
renderer.domElement.addEventListener('contextmenu', e => {
  ...
  if (monster) {
    ...
    monster.mesh.position.x += kb.x*1.5; monster.mesh.position.z += kb.z*1.5;
    if (monster.hp <= 0) { scene.remove(monster.mesh); monsters.splice(monsters.indexOf(monster),1); }
    return;
  }
  const hit = rayHitBlock();
  if (hit) {
    ...
    if (block.userData.type !== 'lava') {
      ...
      if (block.userData.hp <= 0) { scene.remove(block); placedBlocks.splice(placedBlocks.indexOf(block),1); }
    }
  }
});
...
let night = false, wasNight = false, nightsSurvived = 0;
    ...
    if (m.mesh.position.distanceTo(camera.position) < 1.3) {
      ...
      if (m.attackTimer > 60 && player.invincible <= 0) {
        ...
        if (player.hp <= 0) {
          ...
          document.getElementById('overlay').innerHTML = '<h1 style="color:#e94560">YOU DIED</h1><p>Refresh to try again</p>';
        ...
        if (m.attackTimer > 80) {
          ...
          if (v.hp <= 0) {
            ...
            if (villagers.length === 0) {
              document.exitPointerLock();
              document.getElementById('overlay').innerHTML = '<h1 style="color:#e94560">VILLAGE FELL</h1><p>All villagers died.</p>';
              document.getElementById('overlay').style.display = 'flex';
            }
          }
        }
...
function updateHUD() {
  ...
  document.getElementById('status').textContent = `${phase}  |  Villagers: ${villagers.length}  |  Monsters: ${monsters.length}  |  Nights survived: ${nightsSurvived}`;
}
